...
MINOVERLAP = 0.5
GT_PATH = './accuracy/groundtruths'
DR_PATH = './accuracy/detections'
...
for txt_file in ground_truth_files_list:
    ...
    temp_path = os.path.join(DR_PATH, (file_id + ".txt"))  # 文件路径
    lines_list = file_lines_to_list(txt_file)  # 读取文件中的每一行数据

    bounding_boxes = []
    for line in lines_list:  # 遍历当前文件中的所有行
        class_name, left, top, right, bottom = line.split()  # 解析具体的内容
...
gt_classes = sorted(list(gt_counter_per_class.keys()))  # 所有的类别
n_classes = len(gt_classes)  # 类别个数
...
print(text)
print("pr,precision,recall 都被保存在accuracy文件夹中")